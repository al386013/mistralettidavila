...
{
   try {
      $query = "INSERT INTO $table (nombre, descripcion, localizacion, fecha, hora, foto_file, usuario) VALUES (?,?,?,?,?,?,?)";
      $consult = $pdo->prepare($query);
...
{
   try {
      $query = "UPDATE $table SET nombre=(?),descripcion=(?),localizacion=(?),fecha=(?),hora=(?),foto_file=(?) WHERE actividad_id=(?)";
      $consult = $pdo->prepare($query);
      $a = $consult->execute($valores);
