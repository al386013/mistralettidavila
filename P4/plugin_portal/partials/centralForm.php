...
<main role="main">
	...
	<form enctype="multipart/form-data" class="fom_usuario" action="?action=actividades&proceso=<?php echo (!is_null($_REQUEST['id']) and $_REQUEST['id'] > 0) ? 'modificar2' : 'registrar'; ?>" method="POST">
		...
		<input type="time" id="hora" name="hora" value="<?php print $hora?>" required/>
                <input type="hidden" id="oculto_nombre" name="oculto_nombre" value="<?php print $oculto_nombre ?>"/>
		<br/>
                <label for="hora">Foto de la actividad</label>
                <ul class="info">
                  <li>Sube una foto con extensión png, jpg o jpeg.</li>
                  <li>Tamaño máximo de 700KB.</li>
                </ul>
                <input type="file" accept="image/*" name="foto_actividad" id="upload">
                <div id="foto_form"><img id="imagen_form"></div>
